...
2. ✅ **Loads** the appropriate step instruction from `references/steps/XX-name.md`
...
└── scripts/
    └── upload-to-cloud.sh          # Upload file to cloud (optional)
...
STEP_FILE="references/steps/$(printf '%02d' ${CURRENT_STEP})-*.md"
...
```bash
# Read the init step
cat references/steps/00-init.md
```
...
```bash
# Find the step file
STEP_FILE=$(ls references/steps/$(printf '%02d' ${NEXT_STEP})-*.md)
...
1. Load `references/steps/08-package.md`
...
**Step files in `references/steps/` directory:**
...
```
Solution: Check that references/steps/XX-name.md files exist
         Use ls references/steps/ to verify
```

**Issue:** "Invalid state file"
```
Solution: Run scripts/validate-state.sh
